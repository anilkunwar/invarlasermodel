...
def calculate_specific_enthalpy(df, element_composition):
    # Molar weights of Fe and Ni
    molar_weights = {'Fe': 55.845, 'Ni': 58.693}
    
    # Calculate molar weight of the alloy
...
def main():
    st.title("Fe-Ni Alloy Specific Enthalpy Calculation. Choose xFe=0.64 and xNi=0.36 for Invar Alloy")
    ...
    if uploaded_file is not None:
        ...
        elements = ['Fe', 'Ni']  # Fe-Ni system
        element_composition = {}
        for element in elements:
            mole_fraction = st.number_input(f"Mole Fraction of {element}", min_value=0.0, max_value=1.0, value=0.5)
